...
import app.pulse.compose.routing.CallbackPredictiveBackHandler
...
internal constructor(
    ...
    companion object {
        val TactileSpringSpec = spring<Dp>(
            dampingRatio = 0.86f,
            stiffness = 420f
        )
    }
    ...
    private fun deferAnimateTo(
        newValue: Dp,
        spec: AnimationSpec<Dp> = TactileSpringSpec
    ) = coroutineScope.launch {
        animatable.animateTo(newValue, spec)
    }

    private fun collapse(spec: AnimationSpec<Dp> = TactileSpringSpec) {
        onAnchorChanged(Anchor.Collapsed)
        deferAnimateTo(collapsedBound, spec)
    }

    private fun expand(spec: AnimationSpec<Dp> = TactileSpringSpec) {
        onAnchorChanged(Anchor.Expanded)
        deferAnimateTo(expandedBound, spec)
    }

    private fun dismiss(spec: AnimationSpec<Dp> = TactileSpringSpec) {
    ...
    fun collapseSoft() = collapse(spring<Dp>(dampingRatio = 0.8f, stiffness = 380f))
