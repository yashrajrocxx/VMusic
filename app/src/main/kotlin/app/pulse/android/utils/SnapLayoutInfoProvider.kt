...
class GridSnapLayoutInfoProvider(
    ...
    override fun calculateSnapOffset(velocity: Float): Float {
        ...
        return when {
            velocity < 0 -> if (lowerBoundOffset != Float.NEGATIVE_INFINITY) lowerBoundOffset else 0f
            velocity > 0 -> if (upperBoundOffset != Float.POSITIVE_INFINITY) upperBoundOffset else 0f
            else -> {
                if (lowerBoundOffset != Float.NEGATIVE_INFINITY && upperBoundOffset != Float.POSITIVE_INFINITY) {
                    if (lowerBoundOffset * -1f > upperBoundOffset) upperBoundOffset else lowerBoundOffset
                } else if (lowerBoundOffset != Float.NEGATIVE_INFINITY) {
                    lowerBoundOffset
                } else if (upperBoundOffset != Float.POSITIVE_INFINITY) {
                    upperBoundOffset
                } else {
                    0f
                }
...
class ListSnapLayoutInfoProvider(
    ...
    override fun calculateSnapOffset(velocity: Float): Float {
        ...
        layoutInfo.visibleItemsInfo.fastForEach { item ->
            ...
            // Closest item before/at center
            if (offset <= 0f && offset > lowerBoundOffset) lowerBoundOffset = offset
            // Closest item after/at center
            if (offset >= 0f && offset < upperBoundOffset) upperBoundOffset = offset
        }

        return when {
            velocity < 0f -> if (lowerBoundOffset != Float.NEGATIVE_INFINITY) lowerBoundOffset else 0f
            velocity > 0f -> if (upperBoundOffset != Float.POSITIVE_INFINITY) upperBoundOffset else 0f
            else -> {
                if (lowerBoundOffset != Float.NEGATIVE_INFINITY && upperBoundOffset != Float.POSITIVE_INFINITY) {
                    if (kotlin.math.abs(lowerBoundOffset) < upperBoundOffset) lowerBoundOffset else upperBoundOffset
                } else if (lowerBoundOffset != Float.NEGATIVE_INFINITY) {
                    lowerBoundOffset
                } else if (upperBoundOffset != Float.POSITIVE_INFINITY) {
                    upperBoundOffset
...
@Composable
fun rememberListSnapLayoutInfo(
    lazyListState: LazyListState
): SnapLayoutInfoProvider {
    return remember(lazyListState) {
        ListSnapLayoutInfoProvider(lazyListState = lazyListState)
    }
}
