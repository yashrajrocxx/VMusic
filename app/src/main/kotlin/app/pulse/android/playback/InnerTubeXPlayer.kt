...
import app.pulse.android.playback.potoken.PoTokenGenerator
...
object InnerTubeXPlayer {
    ...
    suspend fun playerResponseForPlayback(
        ...
        try {
            ...
            val stream =
                ...
                ) { "InnerTubeX returned no playable stream for $videoId" }
    ...
    private suspend fun bundle(): ExtractionBundle {
        ...
        return bundleMutex.withLock {
            ...
            val innerTube = InnerTubeX(client)
            val remoteStore = RemotePlayerConfigStore(client, configRepository, logger)
            val cipherService = YouTubeCipherService(client, remoteStore, logger)
            val extractor =
                InnerTubeExtractor(
                    configParser =
                        YtConfigParserImpl(
    ...
    private fun createHttpClient() = HttpClient(OkHttp) {
        expectSuccess = false
        defaultRequest {
            if (AccountPreferences.isLoggedIn) {
                headers.append("cookie", AccountPreferences.innerTubeCookie)
                if (AccountPreferences.visitorData.isNotBlank()) {
                    headers.append("X-Goog-Visitor-Id", AccountPreferences.visitorData)
                }
                if (AccountPreferences.dataSyncId.isNotBlank()) {
                    headers.append("X-Youtube-Datasync-Id", AccountPreferences.dataSyncId)
                }
            }
        }
